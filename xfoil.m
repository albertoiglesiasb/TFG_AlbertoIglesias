function coef = xfoil(coord, alpha, Re, Mach, varargin)
    % Run XFoil and return the lift and drag coefficients (Cl and Cd).
    ...
    %
    % Outputs:
    %   coef: structure with fields 'alpha', 'Cl', and 'Cd'
    ...
    if ~iter_set
        varargin = [varargin, {'oper iter 150'}];
    end
    ...
    if fid <= 0
        ...
        t = timer('StartDelay', 20, 'TasksToExecute', 1, 'ExecutionMode', 'fixedRate');
        ...
        try
            ...
            warning(ME.message);  % Re-throw the last error
        end
